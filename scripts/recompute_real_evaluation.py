...
def run_classical_preprocessing(queries):
    ...
    classical_results = preprocessor.preprocess_batch(
        raw_query_texts,
    ...
    classical_queries = {
        raw_query_ids[i]: classical_results[i][0] for i in range(len(raw_query_ids))
    }
    print(
        f"[Prep] Preprocessed {len(raw_query_ids)} queries in {time.time() - t0:.2f} seconds."
    )
...
def run_neural_preprocessing(queries):
    ...
    raw_query_ids = list(queries.keys())
    raw_query_texts = [queries[qid] for qid in raw_query_ids]
    neural_results = preprocessor.preprocess_batch(
        raw_query_texts, pipeline_type="neural", verbose=False
    )
    neural_queries = {
        raw_query_ids[i]: neural_results[i][0] for i in range(len(raw_query_ids))
    }
    print(
        f"[Prep] Preprocessed {len(raw_query_ids)} queries in {time.time() - t0:.2f} seconds."
    )
...
def evaluate_vsm(dataset_name, queries, qrels):
    ...
    for b_start in range(0, len(qids), batch_size):
        ...
        b_vecs = vectorizer.transform(b_texts).astype(
            np.float32
        )  # Shape: (batch_size, terms)
    ...
    print(
        f"[VSM] Dataset={dataset_name} | MAP: {avg_map * 100:.2f}%, Recall: {avg_recall * 100:.2f}%, P@10: {avg_p10 * 100:.2f}%, NDCG: {avg_ndcg * 100:.2f}%"
    )
    return {
...
def evaluate_bm25(dataset_name, queries, qrels):
    ...
    for qid in qids:
        ...
        bm25_top100[qid] = [
            {"id": str(doc_ids[idx_i]), "score": float(scores[idx_i])}
    ...
    print(
        f"[BM25] Dataset={dataset_name} | MAP: {avg_map * 100:.2f}%, Recall: {avg_recall * 100:.2f}%, P@10: {avg_p10 * 100:.2f}%, NDCG: {avg_ndcg * 100:.2f}%"
    )
    return {
        "precision_at_k": round(avg_p10, 4),
        "recall_score": round(avg_recall, 4),
...
def evaluate_embedding(dataset_name, queries, qrels):
    ...
    print(
        "[Embedding] Calculating cosine similarities in batches (RAM-optimized batch size 20)..."
    )
    batch_size = 20
    emb_top100 = {}
    emb_runs = {}
    ...
    for b_start in range(0, len(qids), batch_size):
        ...
        for idx_in_batch, qid in enumerate(b_qids):
            ...
            emb_top100[qid] = [
                {"id": str(doc_ids[idx_i]), "score": float(q_scores[idx_i])}
                for idx_i in top_idx
            ]
    ...
    print(
        f"[Embedding] Dataset={dataset_name} | MAP: {avg_map * 100:.2f}%, Recall: {avg_recall * 100:.2f}%, P@10: {avg_p10 * 100:.2f}%, NDCG: {avg_ndcg * 100:.2f}%"
    )
    return {
        "precision_at_k": round(avg_p10, 4),
        "recall_score": round(avg_recall, 4),
...
def evaluate_hybrid(dataset_name, queries, qrels):
    ...
    embedding_run_path = os.path.join(
        os.path.dirname(CACHE_PATH), f"temp_runs_embedding_{dataset_name}.joblib"
    )

    if not os.path.exists(bm25_run_path) or not os.path.exists(embedding_run_path):
        print(
            f"[Hybrid] Warning: BM25 or Embedding runs not found at {bm25_run_path} / {embedding_run_path}"
        )
        return None
    ...
    for qid in qids:
        ...
        # Parallel Hybrid: RRF
        parallel_fused = reciprocal_rank_fusion(
            [bm25_res, dense_res], weights=[0.5, 0.5]
        )[:10]
    ...
    for qid in hybrid_serial_runs:
        p10_list.append(
            evaluator.calculate_precision_at_k(hybrid_serial_runs[qid], qid, k=10)
        )
    ...
    for qid in hybrid_parallel_runs:
        p10_list.append(
            evaluator.calculate_precision_at_k(hybrid_parallel_runs[qid], qid, k=10)
        )
    ...
    print(
        f"[Hybrid] Dataset={dataset_name} | SERIAL -> MAP: {avg_map_s * 100:.2f}%, Recall: {avg_rec_s * 100:.2f}%, P@10: {avg_p10_s * 100:.2f}%, NDCG: {avg_ndcg_s * 100:.2f}%"
    )
    print(
        f"[Hybrid] Dataset={dataset_name} | PARALLEL -> MAP: {avg_map_p * 100:.2f}%, Recall: {avg_rec_p * 100:.2f}%, P@10: {avg_p10_p * 100:.2f}%, NDCG: {avg_ndcg_p * 100:.2f}%"
    )
    return {
        "hybrid_serial": {
...
def run_method(method):
    ...
    temp_path = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "..", "data", f"temp_metrics_{method}.json"
        )
    )
...
def run_coordinator():
    ...
    print(
        "=== [COORDINATOR] STARTING SUBPROCESS-ISOLATED FULL EVALUATION RECOMPUTATION ==="
    )
    print(
        "================================================================================"
    )
            ...
            if os.path.exists(temp_path):
                try:
                    ...
                    if ds in method_metrics:
                        if method == "hybrid":
                            results[ds]["hybrid_serial"] = method_metrics[ds][
                                "hybrid_serial"
                            ]
                            results[ds]["hybrid_parallel"] = method_metrics[ds][
                                "hybrid_parallel"
                            ]
                        else:
    ...
    for method in methods:
        ...
        if os.path.exists(temp_path):
            os.remove(temp_path)
        for ds in datasets:
            temp_run_path = os.path.abspath(
                os.path.join(
                    os.path.dirname(__file__),
                    "..",
                    "data",
                    f"temp_runs_{method}_{ds}.joblib",
                )
            )
    ...
    # ✅ الإضافة الوحيدة: ملخص النتائج النهائي لكل داتا سيت (بدون أي تعديل على الكود الأساسي)
    print("\nSummary of BASE evaluation results (from cache):")
    for ds in datasets:
        print(f"\n  Dataset: {ds}")
        for model in ["vsm", "bm25", "embedding", "hybrid_serial", "hybrid_parallel"]:
            if model in results[ds]:
                m = results[ds][model]
                if isinstance(m, dict) and "map_score" in m:
                    print(
                        f"    {model:20s} MAP={m.get('map_score', '?'):.4f}  "
                        f"nDCG={m.get('ndcg_score', '?'):.4f}"
                    )
