...
class Settings:
    ...
    INDICES_DIR = os.path.abspath(
        os.path.join(BASE_DIR, "services", "indexing_service", "data", "indices")
    )
    ...
    # تفعيل/تعطيل التدقيق النحوي (يحتاج مكتبة language-tool-python وJava)
    ENABLE_GRAMMAR_CHECK = os.getenv("ENABLE_GRAMMAR_CHECK", "false").lower() in (
        "1",
        "true",
        "yes",
    )
