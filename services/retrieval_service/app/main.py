    ...
    def get_searchers(self, dataset: str, method: str):
        ...
        if method in ["vsm", "bm25", "hybrid_serial", "hybrid_parallel"]:
            if dataset not in self.index_cache:
                ...
                if os.path.exists(index_path):
                    ...
                    print(
                        f"[ModelManager] Warning: Inverted Index file not found at {index_path}"
                    )
        ...
        if method == "vsm":
            if dataset not in self.vsm_cache:
                model_path = os.path.join(
                    settings.MODELS_DIR, f"{dataset}_tfidf.joblib"
                )
        ...
        # BM25
            ...
            if dataset not in self.bm25_cache:
                ...
                if not os.path.exists(model_path):
                    raise FileNotFoundError(
                        f"BM25 model file not found at: {model_path}"
                    )
        ...
        # BERT Embeddings
            ...
            if dataset not in self.vector_cache:
                model_path = os.path.join(
                    settings.MODELS_DIR, f"{dataset}_embedding.joblib"
                )
                if not os.path.exists(model_path):
                    raise FileNotFoundError(
                        f"BERT embeddings file not found at: {model_path}"
                    )
            ...
            if self.bert_model is None:
                print(
                    "[ModelManager] Loading SentenceTransformer model 'all-MiniLM-L6-v2'..."
                )
        ...
        # Hybrid Serial / Hybrid Parallel
            ...
            hybrid_searcher = HybridSearcher(
                self.bm25_cache[dataset], self.vector_cache[dataset]
            )
...
def preload_models():
    print(
        "--- [Startup] Preloading all search indexes and models into RAM (8 GB RAM compliant) ---"
    )
...
def search_documents(request: QueryRequest):
    ...
    try:
        ...
        if method in ["embedding", "hybrid_serial", "hybrid_parallel"]:
            ...
            query_emb = bert_model.encode(
                request.query, convert_to_numpy=True, normalize_embeddings=True
            )
            ...
            if request.use_additional_features:
                ...
                try:
                    ...
                    history_queries = [
                        row[0]
                        for row in rows
                        if row[0].strip().lower() != request.query.strip().lower()
                    ]
                    if history_queries:
                        history_embs = [
                            bert_model.encode(
                                hq, convert_to_numpy=True, normalize_embeddings=True
                            )
                            for hq in history_queries
                        ]
                        user_interest_vector = np.mean(history_embs, axis=0)
                        user_interest_vector = user_interest_vector / np.linalg.norm(
                            user_interest_vector
                        )

                        fusion_alpha = 0.7
                        query_emb = (
                            fusion_alpha * query_emb
                            + (1.0 - fusion_alpha) * user_interest_vector
                        )
                        ...
                        print(
                            f"[VectorFusion] Query vector fused with user interests from history: {history_queries}"
                        )
        ...
        if method == "vsm":
            raw_results = searcher.search(
                request.query, top_k=request.top_k, inverted_index=inverted_idx
            )
        elif method == "bm25":
            ...
            bm25_query = (
                request.preprocessed_query
                if request.preprocessed_query
                else request.query
            )
            query_tokens = bm25_query.split()
            # serial: sparse candidates, then dense rerank
            # ✅ تم تمرير hybrid_alpha هنا
            ...
            raw_results = searcher.search_serial(
                query_tokens,
                query_emb,
                top_k=request.top_k,
                inverted_index=inverted_idx,
            ...
            if bm25_searcher:
                ...
                if request.bm25_b is not None:
                    bm25_searcher.bm25.b = request.bm25_b
            raw_results = searcher.search_parallel(
                query_tokens,
                query_emb,
                top_k=request.top_k,
                inverted_index=inverted_idx,
        ...
        if not request.retrieve_text:
            ...
            results = [
                DocumentSchema(id=d_id, content="", score=scores_dict.get(d_id, 0.0))
                for d_id in doc_ids
            ]
            time_taken_ms = (time.time() - start_time) * 1000
            return {
        ...
        for d_id in doc_ids:
            ...
            results.append(DocumentSchema(id=d_id, content=text, score=score))
...
def search_documents_batch(request: BatchQueryRequest):
    ...
    try:
        ...
        preprocessed_queries = (
            request.preprocessed_queries if request.preprocessed_queries else queries
        )
        ...
        if method == "vsm":
            raw_results_list = searcher.search_batch(
                preprocessed_queries, top_k=request.top_k, inverted_index=inverted_idx
            )
        elif method == "bm25":
            ...
            query_embs = bert_model.encode(
                queries, convert_to_numpy=True, normalize_embeddings=True
            )
            query_embs = query_embs.astype("float32")
            raw_results_list = searcher.search_batch(query_embs, top_k=request.top_k)
        elif method == "hybrid_serial":
            queries_tokens = [q.split() for q in preprocessed_queries]
            query_embs = bert_model.encode(
                queries, convert_to_numpy=True, normalize_embeddings=True
        ...
        for q_idx, raw_results in enumerate(raw_results_list):
            ...
            batch_results.append(
                BatchQueryResult(query=queries[q_idx], results=results)
            )

        time_taken_ms = (time.time() - start_time) * 1000
        return BatchSearchResponse(results=batch_results, time_taken_ms=time_taken_ms)
    except Exception as e:
        import traceback
