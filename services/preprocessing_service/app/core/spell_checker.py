...
# preprocessing_service/app/core/spell_checker.py
...
class TextSpellChecker:
    ...
    def init(self, max_edit_distance: int = 2):
        ...
        if not TextSpellChecker._valid_words:
            try:
                ...
                nltk.download("words", quiet=True)
            TextSpellChecker._valid_words = set(nltk.corpus.words.words())
            # Optionally inject project‑specific terms here:
            # TextSpellChecker._valid_words.update(["myterm1", "myterm2"])
