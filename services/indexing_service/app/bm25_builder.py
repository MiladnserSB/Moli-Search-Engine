...
def build_bm25_representation(dataset_name: str, k1: float = 1.5, b: float = 0.75) -> dict:
    ...
    # 2. Tokenize corpus
    print("\n[TOKENIZATION] Tokenizing preprocessed corpus (splitting by whitespace)...")
    tok_start = time.time()
    tokenized_corpus = [str(text).split() for text in df['processed_text']]
    print(f"[OK] Tokenized {len(tokenized_corpus):,} docs in {time.time() - tok_start:.2f} seconds.")
    
    # 3. Fit BM25 Okapi index
    print("\n[FITTING] Fitting BM25Okapi index (calculating document frequencies and stats)...")
    fit_start = time.time()
    bm25_vectorizer = BM25Okapi(tokenized_corpus, k1=k1, b=b)
    fit_duration = time.time() - fit_start
    print(f"[OK] BM25 index fit completed in {fit_duration:.2f} seconds.")
    ...
    print(f"\n[SAVE] Dumping BM25 model, tokenized corpus, and doc mappings to disk at: {out_path}...")
    ...
    print(f"\n[SUCCESS] BM25 Build completed in {elapsed_time:.2f} seconds.")
    print("="*60 + "\n")
